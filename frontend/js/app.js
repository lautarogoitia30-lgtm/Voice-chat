        ...
        participants.forEach(p => {
            ...
            item.innerHTML = `
                ...
                <div class="voice-participant-volume">
                    ...
                    <input type="range" class="user-volume-slider" min="0" max="300" value="${userVol}" 
                        data-user-id="${userId}" 
...
function handleAvatarFileSelect(input) {
    console.log('[AVATAR] File selected:', input.files);
    const file = input.files[0];
    if (!file) return;
    
    console.log('[AVATAR] File details:', file.name, file.type, file.size);
